...
@section('container')
    <form class="max-w-sm" action="/admin/medichine/{{ $medichine->id }}" method="post">
        @method('PUT')
        @csrf
        <div class="mb-5">
            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
            <input type="text" id="name" name="name" value="{{ $medichine->name }}"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
        </div>

        <div class="mb-5">
            <label for="price" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Price</label>
            <input type="text" id="price" name="price" value="{{ $medichine->price }}"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
        </div>
        ...
        <div class="max-w-sm mx-auto mb-5">
            ...
            <select id="id_category" name="id_category"
                ...
                @foreach ($medichine_categories as $mc)
                    <option value="{{ $mc->id }}" {{ $mc->id == $medichine->id_category ? 'selected' : '' }}>
                        {{ $mc->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="max-w-sm mx-auto mb-5">
            ...
            <textarea id="description" name="description" rows="4"
                ...
                placeholder="">{{ $medichine->description }}</textarea>
        </div>
