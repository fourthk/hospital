        ...
        <tbody>
            @foreach ($appointments as $appointment)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 py-4">
                        {{ $status_payment[$appointment->status_payment] }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $hours[$appointment->plan->hour] }}
                    </td>
                    <td class="px-6 py-4">
                        {{ Carbon::parse($appointment->date)->translatedFormat('d F Y') }}
                    </td>
                    <td class="px-6 py-4">
                        @if ($page == 'appointment')
    ...
    <div class="py-4 bg-white dark:bg-gray-900 flex justify-start">
        <div class="flex flex-col items-center">
            ...
            <span class="text-sm text-gray-700 dark:text-gray-400">
                Showing <span class="font-semibold text-gray-900 dark:text-white">{{ $appointments->firstItem() }}</span>
                to <span class="font-semibold text-gray-900 dark:text-white">{{ $appointments->lastItem() }}</span> of
                <span class="font-semibold text-gray-900 dark:text-white">{{ $appointments->total() }}</span> Entries
            </span>
        </div>
    </div>
@endsection
