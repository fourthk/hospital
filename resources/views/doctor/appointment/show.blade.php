...
@section('container')
    @php
        use Carbon\Carbon;
    @endphp
    <form action="/doctor/appointment/{{ $appointment->id }}" method="post">
        @method('PUT')
        @csrf
        <div class="max-w-2xl grid grid-cols-2 gap-4">
            ...
            <div class="">
                ...
                <input type="text" id="id" name="id" value="{{ $appointment->id }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
            </div>
            <div class="">
                <label for="no_appointment" class="block mb-2 text-sm font-medium text-gray-900 ">No.
                    Appointment</label>
                <input type="text" id="no_appointment" name="no_appointment" value="{{ $appointment->no_appointment }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
            </div>
            <div class="">
                <label for="doctor" class="block mb-2 text-sm font-medium text-gray-900 ">Doctor</label>
                <input type="text" id="doctor" name="doctor" value="{{ $appointment->doctor->user->fullname }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
            </div>
            <div class="">
                <label for="plan" class="block mb-2 text-sm font-medium text-gray-900 ">plan</label>
                <input type="text" id="plan" name="plan" value="{{ $hours[$appointment->id_plan] }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
            </div>
            <div class="">
                <label for="telephone" class="block mb-2 text-sm font-medium text-gray-900 ">telephone</label>
                <input type="text" id="telephone" name="telephone" value="{{ $appointment->telephone }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
            </div>
            <div class="">
            ...
            <div class="">
                ...
                <select id="status" name="status"
                    ...
                    @foreach ($status as $index => $sts)
                        <option value="{{ $index }}" {{ $index == $appointment->status ? 'selected' : '' }}>
                            {{ $sts }}</option>
                    @endforeach
                </select>
            </div>

            <input type="text"  name="status_payment" value="{{ $appointment->status_payment }}" hidden>

            <div class="">
                <label for="" class="block mb-2 text-sm font-medium text-gray-900 ">Payment
                    Status</label>
