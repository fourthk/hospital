...
@section('container')
    <form class="max-w-sm" action="/doctor/plan/{{ $plan->id }}" method="post">
        @method('PUT')
        @csrf
        <input type="text" name="id_doctor" value="{{ $plan->id_doctor }}" hidden>
        ...
        <div class="max-w-sm mx-auto mb-5">
            ...
            <select id="day" name="day"
                ...
                @foreach ($days as $index => $day)
                        <option value="{{ $index }}" {{ $index == $plan->day ? 'selected' : '' }}>{{ $day }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="max-w-sm mx-auto mb-5">
            ...
            <select id="hour" name="hour"
                ...
                @foreach ($hours as $index => $hour)
                        <option value="{{ $index }}" {{ $index == $plan->hour ? 'selected' : '' }}>{{ $hour }}</option>
                @endforeach
            </select>
        </div>
